...
def show_match_overview(match_id):
    ...
    try:
        weather = requests.get(f'https://api.openweathermap.org/data/2.5/onecall?lat={51.21989}&lon={4.40346}&exclude=current,minutely,hourly,alerts&appid={API_KEY}').json()
    except:
        return "poop"
    return json.dumps(weather)
    return render_template("match_overview.html", match = match, full_names = full_names, home_team_track_record=home_team_tr, away_team_track_record=away_team_tr, statistics=statistics, prev_matches=prev_matches)
